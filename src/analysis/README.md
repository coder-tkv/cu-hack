...
`report` — внутренний отчёт детекторов. Для HTTP API его преобразует `src/reports/builder.py`; контракт сайта находится в `src/schemas/`.
...
### Факт, ограничения и мост к контракту

У каждой находки есть `fact` — одно предложение, целиком собранное из чисел и
цитат лога, и `limitations` — честный список того, чего код установить не смог.
Это блок «ФАКТ — посчитано кодом» из дизайна и тот же текст, который уходит
в промпт: модель объясняет посчитанное, а не придумывает числа.

`candidates.to_candidates(findings, step_id=str)` переводит находки в `Candidate`
из общего контракта (`schemas/findings.py`): закрытый список `kind`, полные
`step_id`, плоский `facts`, `evidence_strength`, `limitations`, `severity=None`
и `rank=None` — их заполняет `ranking.py`. Виды, которых в закрытом списке нет,
возвращаются вторым списком с предложенным `kind`: подменять вид молча нельзя.
...
В отдельном CLI доступны эти рекомендации. Общий backend использует осторожные шаблоны из `reports/builder.py`; он не переносит свободные объяснения причин или настройки разрешений автоматически. ML выбирает факты и допустимый совет по контракту версии 2.
...
```bash
python3 -m unittest discover -s tests -q          # тесты детекторов и интеграции
